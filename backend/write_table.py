...
def update_deal(data):
    ...
    try:
        cur.execute(
    '''
    update kif set customer_name=%s, customer_type=%s, business_type=%s, guarantee=%s, opportunity=%s,
    software=%s, hardware=%s, license=%s, customization=%s, enhancement=%s, deployment=%s, support=%s,
    professional_service=%s, bidding=%s, start_date=%s, end_date=%s, total_contract=%s, region=%s, domain=%s,
    direct_sale=%s, partner=%s, third_party=%s, penalty=%s, acc_manager=%s, presales_spoc=%s, currency=%s,
    cust_budget=%s, travel_expense=%s, tool_license=%s, shift_allowance=%s, weekend_allowance=%s,
    on_call_allowance=%s, one_time_cost=%s, vat=%s, misc=%s) where deal_id=%s;''',
    (
        customer_name,
        customer_type,
        business_type,
